...
const {selectAllUsersFromDatabase, getAllDecksfromDiscord,getMatchHistory,getMatchHistoryUser} = require('../functions/MysqlDataManagementFunctions');
...
const showAllUsersEmbed = async () => {
	...
	const users = await selectAllUsersFromDatabase();
		...
		const userDecks = await getAllDecksfromDiscord(user.Discord_ID).then(decks => {
			let decklists = [];
			if(decks.length < 1) {
				...
				decks.map( deck => {
					decklists.push(
						` \n \n  ** ${deck.Selected ? ":white_check_mark:" : ""} ${deck.Deck_ID}. ${deck.Deck_Name}**
...
const showCertainUserEmbed = async (interaction) => {
	...
	const values = decks.map(function (decklist) {
        return {
          name: `** ${decklist.Selected ? ":white_check_mark:  " : ""} ${
            decklist.Deck_ID
          }. ${decklist.Deck_Name} ** \n   Games played : ${decklist.Games} | Win ratio: ${playerWinPerc(decklist.Wins,decklist.Games)}%`,
  ...
  const userEmbed = new MessageEmbed()
    ...
    .setDescription(
      `Games played : ${games} | General win ratio: ${playerWinPerc(wins,games)}%`
    )
...
const matches = await getMatchHistory(discordId);
  ...
  const userEmbed = new MessageEmbed()
    ...
    .setDescription(
      `Last 20 games played by the discord community`
    )
    .addFields( values );
...
const showMatchHistoryUser = async (interaction,discordId) => {
	...
	const matches = await getMatchHistoryUser(user.id,discordId);
	  ...
	  const userEmbed = new MessageEmbed()
		...
		.setDescription(
		  `Last 20 games played by the player`
		)
		.addFields( values )
		.setThumbnail(user.displayAvatarURL());
...
module.exports = { showAllUsersEmbed, showCertainUserEmbed,showMatchHistoryAll, showMatchHistoryUser };