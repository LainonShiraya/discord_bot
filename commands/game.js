...
const {getUserSelectedDeckfromDatabase, selectUserInDatabase, createGame, assignDecksToGame} = require('../functions/MysqlDataManagementFunctions');
...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      const deck1 = await getUserSelectedDeckfromDatabase(interaction.options.getUser("player_1").id);
      const deck2 = await getUserSelectedDeckfromDatabase(interaction.options.getUser("player_2").id);
      const deck3 = await getUserSelectedDeckfromDatabase(interaction.options.getUser("player_3").id);
	...
	players.options.map(player => {
		...
		if(i.customId === 'ConfirmButton'&& clickedByPlayer){
			...
			const game = await createGame();
				...
				if(!!game){
			assignDecks = await assignDecksToGame(game, playersId[0],playersId[1],playersId[2], winnerId, discordId);
				} else {
			...
			if(game){
				...
				i.reply(`There is a problem with adding the game, contact the bot administrator`);
			}
		  }
	  ...
	  if(!clickedByPlayer){
		i.reply(`${player_interacting} did not participate in the game, game has to be confirmed by one of the players`);
	  }
    });
  },
};
